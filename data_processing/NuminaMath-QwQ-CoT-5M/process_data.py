#!/usr/bin/env python3
"""finds the word distribution of the thinking and non-thinking text in the NuminaMath-QwQ-CoT-5M dataset"""
...
def main(save_dict, plot, individual):
    """process data for NuminaMath-QwQ-CoT-5M dataset"""
    ...
    data_path = Path("data") / "NuminaMath-QwQ-CoT-5M.jsonl"
    ...
    # extracting llm-outputs
    text_chunks = []
    for entry in json_data:
        text_chunks.append(entry["response"])

    # prevent individual plots. option preserved for compatability with shell automation
    if individual:
        print("Warning: Setting individual=false as the data in NuminaMath-QwQ-CoT-5M does not have a distinction between regular and thinking text")
        individual=False

    # Process and analyze using shared utility function
    process_utils.process_and_analyze_text_chunks(
        text_chunks=text_chunks,
        dataset_name="NuminaMath-QwQ-CoT-5M",
